...
- **独立配置**：每名玩家的工作模式、笔刷、标高等设置各自独立；本地保存后会同步到服务端供该玩家操作使用，不会广播给其他玩家
...
配置保存在本地 `config/pushdozer_config.json`，修改后会通过 `ConfigSyncPayload` 同步到服务端，供你的操作使用（工作模式、笔刷尺寸、标高等）。每位玩家独立一份，**不会广播给其他玩家**，也不会写入他人的本地文件。
